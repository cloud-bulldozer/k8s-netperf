...
func ShowPodCPU(s ScenarioResults) {
	table := initTable([]string{"Result Type", "Pair", "Driver", "Role", "Scenario", "Parallelism", "Host Network", "Service", "External Server", "UDN Info", "Bridge Info", "Message Size", "Burst", "Same node", "Pod", "Utilization"})
	for _, r := range s.Results {
		for _, pod := range r.ClientPodCPU.Results {
			table.Append([]string{"Pod CPU Utilization", strconv.Itoa(r.PairIndex), r.Driver, "Client", r.Profile, fmt.Sprintf("%d", r.Parallelism), fmt.Sprintf("%t", r.HostNetwork), fmt.Sprintf("%t", r.Service), fmt.Sprintf("%t", r.ExternalServer), r.UdnInfo, r.BridgeInfo, fmt.Sprintf("%d", r.MessageSize), fmt.Sprintf("%d", r.Burst), fmt.Sprintf("%t", r.SameNode), fmt.Sprintf("%.20s", pod.Name), fmt.Sprintf("%f", pod.Value)})
		}
		for _, pod := range r.ServerPodCPU.Results {
			table.Append([]string{"Pod CPU Utilization", strconv.Itoa(r.PairIndex), r.Driver, "Server", r.Profile, fmt.Sprintf("%d", r.Parallelism), fmt.Sprintf("%t", r.HostNetwork), fmt.Sprintf("%t", r.Service), fmt.Sprintf("%t", r.ExternalServer), r.UdnInfo, r.BridgeInfo, fmt.Sprintf("%d", r.MessageSize), fmt.Sprintf("%d", r.Burst), fmt.Sprintf("%t", r.SameNode), fmt.Sprintf("%.20s", pod.Name), fmt.Sprintf("%f", pod.Value)})
		}
	}
	table.Render()
}
...
func ShowPodMem(s ScenarioResults) {
	...
	for _, r := range s.Results {
		for _, pod := range r.ClientPodMem.MemResults {
			table.Append([]string{"Pod Mem RSS Utilization", strconv.Itoa(r.PairIndex), r.Driver, "Client", r.Profile, fmt.Sprintf("%d", r.Parallelism), fmt.Sprintf("%t", r.HostNetwork), fmt.Sprintf("%t", r.Service), fmt.Sprintf("%t", r.ExternalServer), r.UdnInfo, r.BridgeInfo, fmt.Sprintf("%d", r.MessageSize), fmt.Sprintf("%d", r.Burst), fmt.Sprintf("%t", r.SameNode), fmt.Sprintf("%.20s", pod.Name), fmt.Sprintf("%f", pod.Value)})
		}
...
func ShowNodeCPU(s ScenarioResults) {
	table := initTable([]string{"Result Type", "Pair", "Driver", "Role", "Scenario", "Parallelism", "Host Network", "Service", "External Server", "UDN Info", "Bridge Info", "Message Size", "Burst", "Same node", "Idle CPU", "User CPU", "System CPU", "Steal CPU", "IOWait CPU", "Nice CPU", "SoftIRQ CPU", "IRQ CPU"})
	for _, r := range s.Results {
		...
		table.Append([]string{
			"Node CPU Utilization", strconv.Itoa(r.PairIndex), r.Driver, "Client", r.Profile, fmt.Sprintf("%d", r.Parallelism), fmt.Sprintf("%t", r.HostNetwork), fmt.Sprintf("%t", r.Service), fmt.Sprintf("%t", r.ExternalServer), r.UdnInfo, r.BridgeInfo, fmt.Sprintf("%d", r.MessageSize), fmt.Sprintf("%d", r.Burst), fmt.Sprintf("%t", r.SameNode),
		...
		table.Append([]string{
			"Node CPU Utilization", strconv.Itoa(r.PairIndex), r.Driver, "Server", r.Profile, fmt.Sprintf("%d", r.Parallelism), fmt.Sprintf("%t", r.HostNetwork), fmt.Sprintf("%t", r.Service), fmt.Sprintf("%t", r.ExternalServer), r.UdnInfo, r.BridgeInfo, fmt.Sprintf("%d", r.MessageSize), fmt.Sprintf("%d", r.Burst), fmt.Sprintf("%t", r.SameNode),
...
func renderResults(s ScenarioResults, testType string) {
	table := initTable([]string{"Result Type", "Pair", "Driver", "Scenario", "Parallelism", "Host Network", "Service", "External Server", "UDN Info", "Bridge Info", "Message Size", "Burst", "Same node", "Duration", "Samples", "Avg value", "95% Confidence Interval"})
	for _, r := range s.Results {
		if strings.Contains(r.Profile, testType) {
			if len(r.Driver) > 0 {
				...
				table.Append([]string{fmt.Sprintf("📊 %s Results", caser.String(strings.ToLower(testType))), strconv.Itoa(r.PairIndex), r.Driver, r.Profile, strconv.Itoa(r.Parallelism), strconv.FormatBool(r.HostNetwork), strconv.FormatBool(r.Service), fmt.Sprintf("%t", r.ExternalServer), r.UdnInfo, r.BridgeInfo, strconv.Itoa(r.MessageSize), strconv.Itoa(r.Burst), strconv.FormatBool(r.SameNode), strconv.Itoa(r.Duration), strconv.Itoa(r.Samples), fmt.Sprintf("%f (%s)", avg, r.Metric), fmt.Sprintf("%f-%f (%s)", lo, hi, r.Metric)})
			}
		}
	}
	table.Render()
...
func ShowLatencyResult(s ScenarioResults) {
	if checkResults(s, "RR") {
		...
		table := initTable([]string{"Result Type", "Pair", "Driver", "Scenario", "Parallelism", "Host Network", "Service", "External Server", "UDN Info", "Bridge Info", "Message Size", "Burst", "Same node", "Duration", "Samples", "Avg 99%tile value"})
		for _, r := range s.Results {
			if strings.Contains(r.Profile, "RR") {
				p99, _ := Average(r.LatencySummary)
				table.Append([]string{"RR Latency Results", strconv.Itoa(r.PairIndex), r.Driver, r.Profile, strconv.Itoa(r.Parallelism), strconv.FormatBool(r.HostNetwork), strconv.FormatBool(r.Service), fmt.Sprintf("%t", r.ExternalServer), r.UdnInfo, r.BridgeInfo, strconv.Itoa(r.MessageSize), strconv.Itoa(r.Burst), strconv.FormatBool(r.SameNode), strconv.Itoa(r.Duration), strconv.Itoa(r.Samples), fmt.Sprintf("%f (%s)", p99, "usec")})
			}
		}
		table.Render()
	}
